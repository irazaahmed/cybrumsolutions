...
/** Shared metadata for a post page in any language. */
export function buildPostMetadata(slug: string, lang: Lang): Metadata {
  ...
  const canonicalLang: Lang = post.lang;
  ...
  return {
    ...
    openGraph: {
      ...
      // English pages get the branded per-post image from the file-based
      // opengraph-image.tsx in the [slug] segment. That file does not cascade
      // into the nested [lang] segment, so translations point at the same
      // generated image route explicitly.
      ...(lang !== "en"
        ? {
            images: [
              {
...
/** Full article page body, shared by the English route and the /[lang] route. */
export function PostArticle({ slug, lang }: { slug: string; lang: Lang }) {
  const post = getPostBySlug(slug, lang);
  if (!post) notFound();
